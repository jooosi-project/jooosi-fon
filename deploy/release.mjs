...
function update(relativePath, replacements) {
  ...
  for (const [pattern, replacement] of replacements) {
    if (!pattern.test(content)) throw new Error(`Could not find the version field in ${relativePath}.`);
...
update("jooosi-fon.php", [[/( \* Version:\s+)\d+\.\d+\.\d+/, `$1${version}`]]);
...
const repositoryUrl = "https://github.com/jooosi-project/jooosi-fon";
const comparisonLink = changelog.match(/^\[unreleased]: https:\/\/github\.com\/jooosi-project\/jooosi-fon\/compare\/(.+)\.\.\.HEAD$/m);
const initialLink = `[unreleased]: ${repositoryUrl}/commits/master`;
...
const releaseFiles = ["CHANGELOG.md", "constant.php", "jooosi-fon.php", "package.json", "readme.txt"];
