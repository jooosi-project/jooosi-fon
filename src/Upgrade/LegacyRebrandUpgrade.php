...
final class LegacyRebrandUpgrade
{
    ...
    public function run(): void
    {
        ...
        $fontsComplete = $attachmentsComplete && $this->migrateDirectory(
            $legacyRoot . '/' . self::FONT_DIRECTORY,
            $root . '/' . self::FONT_DIRECTORY,
            false,
            $fontChanged
        );
        $cacheComplete = $this->migrateDirectory(
            $legacyRoot . '/' . self::CACHE_DIRECTORY,
            $root . '/' . self::CACHE_DIRECTORY,
            true,
            $cacheChanged
        );
        $rootComplete = $attachmentsComplete
            && $fontsComplete
            && $cacheComplete
            && $this->migrateDirectory($legacyRoot, $root, false, $otherChanged, [
                self::FONT_DIRECTORY,
                self::CACHE_DIRECTORY,
            ]);

        if ($fontChanged || $cacheChanged) {
            update_option(self::CACHE_REBUILD_OPTION, true, false);
        }
    ...
    private function migrateAttachmentPaths(string $baseDirectory, bool &$changed): bool
    {
        ...
        foreach ($rows as $row) {
            ...
            if ($updated === false && get_post_meta($attachmentId, '_wp_attached_file', true) !== $relativePath) {
                $complete = false;
                continue;
            }
    ...
    private function migrateDirectory(
        ...
        string $directory,
        bool $discardLegacyConflicts,
        bool &$changed,
        array $ignoredEntries = []
    ): bool {
        ...
        foreach ($entries as $entry) {
            ...
            if (in_array($entry, $ignoredEntries, true)) {
                continue;
            }
            ...
            if (! is_file($legacyPath)
                || ! $this->migrateFile($legacyPath, $path, $discardLegacyConflicts, $changed)) {
                $complete = false;
            }
        }
    ...
    private function shouldDeleteLegacyData(): bool
    {
        ...
        return is_array($options)
            && is_array($options['misc'] ?? null)
            && ! empty($options['misc']['delete_legacy_data']);
    }
