...
class Main implements BuilderInterface
{
    public function __construct()
    {
        ...
        add_filter('f!jooosi/fon/core/cache:build_css.append_content', fn ($css, $rows) => $this->filter_append_build_css_content($css, $rows), 1_000_001, 2);
        add_filter('f!jooosi/fon/core/cache:build_css.append_content', fn ($css, $rows) => $this->filter_ensure_block_editor($css, $rows), 1_000_001, 2);
    ...
    public function filter_theme_json_data_theme($theme_json)
    {
        ...
        $new_data = [
            ...
            'settings' => [
                'typography' => [
                    'fontFamilies' => self::merge_font_family_presets(
    ...
    private static function merge_font_family_presets(array $existing, array $incoming): array
    {
        ...
        foreach (array_merge($existing, $incoming) as $preset) {
            $slug = is_array($preset) && isset($preset['slug']) && is_string($preset['slug'])
                ? $preset['slug']
                : '';
