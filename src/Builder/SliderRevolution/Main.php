...
class Main implements BuilderInterface
{
    public function __construct()
    {
        ...
        if (Config::get('builder_integrations.disable_google_fonts.slider_revolution', \true)) {
            add_filter('revslider_data_get_font_familys', fn($rev_fonts) => $this->remove_google_fonts($rev_fonts), 1000001);
        }
        add_filter('revslider_data_get_font_familys', fn($rev_fonts) => $this->get_font_families($rev_fonts), 1000001);
        ...
        add_action('admin_menu', static fn() => AdminPage::add_redirect_submenu_page('revslider'), 1000001);
    }
    ...
    public function get_font_families($rev_fonts)
    {
        ...
        foreach ($fonts as $font) {
            ...
            array_unshift($rev_fonts, ['type' => 'jooosi-fon', 'version' => 'Serif Fonts', 'label' => $font['family']]);
        }
        return $rev_fonts;
    }
