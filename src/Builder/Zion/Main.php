...
namespace JooosiFon\Builder\Zion;
...
class Main implements BuilderInterface
{
    ...
    {
        add_filter('rest_request_after_callbacks', fn($response, array $handler, \WP_REST_Request $wprestRequest) => $this->filter_rest_request_after_callbacks($response, $handler, $wprestRequest), 1000001, 3);
        add_action('zionbuilder/editor/after_scripts', fn() => $this->enqueue_assets());
        add_action('admin_menu', static fn() => AdminPage::add_redirect_submenu_page('zionbuilder'), 1000001);
    }
        ...
        if ($request->get_route() === '/zionbuilder/v1/data-sets' && $response instanceof \WP_REST_Response) {
            ...
            if (array_key_exists('fonts_list', $data)) {
                ...
                foreach ($fonts as $font) {
                    $data['fonts_list']['custom_fonts'][] = ['id' => $font['family'], 'name' => '[Jooosi Fon] ' . $font['title']];
                }
