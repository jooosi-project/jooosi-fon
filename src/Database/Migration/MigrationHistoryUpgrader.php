...
final class MigrationHistoryUpgrader
{
    ...
    private function mergeTables(string $legacyTable, string $table): void
    {
        ...
        $merged = $wpdb->query(
            "INSERT IGNORE INTO `{$table}` (`version`, `executed_at`, `execution_time`)
    ...
    private function migrateNamespace(string $table, string $legacyNamespace, string $namespace): void
    {
        ...
        $legacyVersion = $wpdb->get_var($wpdb->prepare(
            "SELECT `version` FROM `{$table}` WHERE `version` LIKE %s LIMIT 1",
            $like
        ));
        ...
        $inserted = $wpdb->query($wpdb->prepare(
            ...
            FROM `{$table}` WHERE `version` LIKE %s",
            $legacyNamespace,
            $namespace,
            $like
        ));
        ...
        $deleted = $wpdb->query($wpdb->prepare(
            "DELETE FROM `{$table}` WHERE `version` LIKE %s",
            $like
        ));
