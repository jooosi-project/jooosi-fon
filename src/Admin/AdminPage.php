...
class AdminPage
{
    ...
    public static function add_redirect_submenu_page($root_slug)
    {
        add_submenu_page(
            $root_slug,
            __('Jooosi Fon', 'jooosi-fon'),
            __('Jooosi Fon', 'jooosi-fon'),
            'manage_options',
            'jooosi-fon-builder-redirect',
    ...
    public function add_admin_menu()
    {
        $hook = add_menu_page(
            ...
            __('Jooosi Fon', 'jooosi-fon'),
            'manage_options',
            JOOOSI_FON::WP_OPTION,
            fn () => $this->render(),
            'data:image/svg+xml;base64,' . base64_encode(file_get_contents(dirname(JOOOSI_FON::FILE) . '/jooosi-fon.svg')),
            100
        );
    ...
    {
        add_filter('admin_footer_text', static fn ($text) => 'Thank you for using <b>Jooosi Fon</b>! Join us on the <a href="https://www.facebook.com/groups/1142662969627943" target="_blank" rel="noopener noreferrer">Facebook Group</a>.', 1_000_001);
        add_filter('update_footer', static fn ($text) => $text . ' | Jooosi Fon ' . JOOOSI_FON::VERSION, 1_000_001);
    ...
    private function redirect_legacy_page(): void
    {
        if (! current_user_can('manage_options')
            || ! isset($_GET['page'])
            || wp_unslash($_GET['page']) !== JOOOSI_FON::LEGACY_WP_OPTION) {
            return;
        }

        Common::redirect(self::get_page_url(), true);
    }
    ...
    private function enqueue_scripts()
    {
        ...
        $assets->enqueue('resources/main.tsx', [
            'handle' => $handle,
        ...
        wp_localize_script($handle, 'jooosiFon', [
            '_version' => JOOOSI_FON::VERSION,
            '_wpnonce' => wp_create_nonce(JOOOSI_FON::WP_OPTION),
            'option_namespace' => JOOOSI_FON::WP_OPTION,
            'text_domain' => 'jooosi-fon',
            'web_history' => self::get_page_url(),
            'rest_api' => [
                'nonce' => wp_create_nonce('wp_rest'),
                'root' => esc_url_raw(rest_url()),
                'namespace' => JOOOSI_FON::REST_NAMESPACE,
                'url' => esc_url_raw(rest_url(JOOOSI_FON::REST_NAMESPACE)),
            ],
            'assets' => [
                'url' => $assets->url(),
            ],
            'lite_edition' => ! Plugin::is_pro_edition(),
        ]);
    }
