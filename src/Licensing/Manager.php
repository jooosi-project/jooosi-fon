...
final class Manager
{
    ...
    public static function get_integration_args(): array
    {
        return [
            ...
            'url' => JOOOSI_FON::EDD_STORE['store_url'],
            'item_id' => JOOOSI_FON::EDD_STORE['item_id'],
            'version' => JOOOSI_FON::VERSION,
            'file' => JOOOSI_FON::FILE,
    ...
    public function boot_updater(): void
    {
        ...
        new PluginUpdater(
            JOOOSI_FON::EDD_STORE['store_url'],
            [
                'file' => JOOOSI_FON::FILE,
                'item_id' => JOOOSI_FON::EDD_STORE['item_id'],
                'version' => JOOOSI_FON::VERSION,
    ...
    public function activate(string $license_key)
    {
        ...
        if ($license_key === '') {
            return new WP_Error(
                'missing',
                __('Enter a license key.', 'jooosi-fon')
            );
        }
    ...
    public function clear_cache(?array $license_keys = null): void
    {
        ...
        foreach (array_unique($license_keys) as $license_key) {
            $cache_key = md5(wp_json_encode([
                $slug,
                $license_key,
                (int) $this->is_pre_release_enabled(),
            ]));
    ...
    private function request(string $action, string $license_key)
    {
        ...
        if (! is_object($license_data)) {
            return new WP_Error(
                ...
                __('The license server could not be reached.', 'jooosi-fon')
            );
        }
