...
class Option extends AbstractApi implements ApiInterface
{
    ...
    public function register_custom_endpoints(): void
    {
        ...
        register_rest_route(
            self::API_NAMESPACE,
            $this->get_prefix() . '/migrate-legacy',
            [
                'methods' => WP_REST_Server::CREATABLE,
                'callback' => fn (\WP_REST_Request $wprestRequest): \WP_REST_Response => $this->migrate_legacy($wprestRequest),
                'permission_callback' => fn (\WP_REST_Request $wprestRequest): bool => $this->permission_callback($wprestRequest),
            ]
        );
    }
    ...
    public function index(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        ...
        return new WP_REST_Response([
            'options' => $options,
            'legacy_migration' => (new LegacyRebrandUpgrade())->status(),
        ]);
    }
    ...
    public function migrate_legacy(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        try {
            ...
            if (! $status['complete']) {
                return new WP_REST_Response([
                    'message' => __('The Yabe Webfont migration is incomplete. The original data was preserved; try again after resolving the reported file issue.', 'jooosi-fon'),
    ...
    public function store(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        ...
        try {
            ...
            return new WP_REST_Response([
                'message' => __('The settings payload could not be encoded.', 'jooosi-fon'),
            ], 400);
        }

        if (! update_option(JOOOSI_FON::WP_OPTION . '_options', $encoded)
            && get_option(JOOOSI_FON::WP_OPTION . '_options') !== $encoded) {
            return new WP_REST_Response([
                'message' => __('The settings could not be saved.', 'jooosi-fon'),
            ], 500);
        }
