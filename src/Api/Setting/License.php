...
class License extends AbstractApi implements ApiInterface
{
    ...
    private function store(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        ...
        if ($new_license_key !== $old_license['key']) {
            if (! $plugin_updater) {
                return new WP_REST_Response([
                    'message' => __('License management is unavailable in this edition.', 'jooosi-fon'),
                    'license' => $this->get_license(),
                ], 404);
            }
            ...
            $newOption = [
                'key' => $new_license_key,
                'opt_in_pre_release' => (bool) $old_license['opt_in_pre_release'],
            ];

            if (! update_option(JOOOSI_FON::WP_OPTION . '_license', $newOption)
                && get_option(JOOOSI_FON::WP_OPTION . '_license') !== $newOption) {
                return new WP_REST_Response([
                    'message' => __('The license was accepted but could not be saved locally.', 'jooosi-fon'),
                    'license' => $this->get_license(),
                ], 500);
            }
        }
    ...
    {
        $license = get_option(JOOOSI_FON::WP_OPTION . '_license', [
            'key' => '',
