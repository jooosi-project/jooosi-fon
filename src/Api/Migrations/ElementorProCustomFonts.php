...
final class ElementorProCustomFonts extends AbstractApi implements ApiInterface
{
    ...
    public function register_custom_endpoints(): void
    {
        register_rest_route(
            ...
            [
                ...
                'callback' => fn (WP_REST_Request $request): WP_REST_Response => $this->import_fonts($request),
        ...
        register_rest_route(
            ...
            [
                'methods' => WP_REST_Server::CREATABLE,
                'callback' => fn (WP_REST_Request $request): WP_REST_Response => $this->clean_up($request),
                'permission_callback' => fn (WP_REST_Request $request): bool => $this->permission_callback($request),
                'args' => [
                    'confirm' => [
                        'required' => true,
                        'sanitize_callback' => 'rest_sanitize_boolean',
                        'validate_callback' => [RequestValidator::class, 'confirmed'],
                    ],
                ],
            ]
        );
    ...
    public function import_fonts(WP_REST_Request $request): WP_REST_Response
    {
        if (! $this->is_active()) {
            return new WP_REST_Response([
                'message' => __('Elementor Pro Custom Fonts is not activated.', 'jooosi-fon'),
            ], 404);
        }
        ...
        add_filter('upload_mimes', static fn ($mimeTypes) => Upload::upload_mimes($mimeTypes, true), 1_000_001);
        ...
        try {
            foreach ($sourceRecords as $sourceRecord) {
                ...
                $migrationSlug = sprintf(
                    'migration-elementor-pro-%s-%d',
                    $postType === self::FONT_FACE_POST_TYPE ? 'font-face' : 'font',
                    $fontId
                );
                ...
                foreach ($normalized['faces'] as $sourceFace) {
                    ...
                    foreach ($sourceFace['files'] as $sourceFile) {
                        $fileName = sanitize_title_with_dashes(sprintf(
                            '%s-%s-%s-%s-%s',
                            $family,
                            $sourceFace['weight'],
                            $sourceFace['style'],
                    ...
                    $fontFaces[] = [
                        ...
                        'comment' => '',
                        'unicodeRange' => '',
                        'preload' => (bool) $sourceFace['preload'],
                        'files' => $files,
                    ];
                }
            ...
            if ($skipped > 0) {
                ...
                return new WP_REST_Response([
                    'message' => __('Some Elementor Pro font variants had no supported files. Nothing was imported so the source records remain safe.', 'jooosi-fon'),
                    'skipped' => $skipped,
                ], 422);
            }
            ...
            return new WP_REST_Response([
                'message' => __('The Elementor Pro Custom Fonts migration failed; newly created Jooosi Fon records were rolled back.', 'jooosi-fon'),
            ], 500);
        }

        $media->commit();
        FontEvents::dispatch('migration', [
            'source' => 'elementor-pro-custom-fonts',
            'ids' => $insertedIds,
        ], true);

        return new WP_REST_Response([
            'message' => __('Elementor Pro custom font families imported. Source records were preserved.', 'jooosi-fon'),
    ...
    public function clean_up(WP_REST_Request $request): WP_REST_Response
    {
        ...
        $fontIds = array_values(array_unique(array_map(
            static fn (array $sourceRecord): int => (int) $sourceRecord['id'],
            $this->source_records()
        )));
        $trashedIds = [];
        ...
        foreach ($fontIds as $fontId) {
            if ($fontId < 1 || ! wp_trash_post($fontId)) {
                ...
                return new WP_REST_Response([
                    'message' => __('Elementor Pro source cleanup failed and previously trashed records were restored.', 'jooosi-fon'),
                ], 500);
            }
        ...
        FontEvents::dispatch('clean_up', [
            'source' => 'elementor-pro-custom-fonts',
            'ids' => $trashedIds,
        ], true);

        return new WP_REST_Response([
            'message' => __('Elementor Pro custom-font records moved to the trash. Original media files were left in place.', 'jooosi-fon'),
            'cleaned' => count($trashedIds),
        ]);
    }
    ...
    private function is_active(): bool
    {
        return defined('ELEMENTOR_PRO_VERSION')
            && (post_type_exists(self::LEGACY_POST_TYPE) || post_type_exists(self::FONT_FACE_POST_TYPE));
    }
    ...
    private function source_records(): array
    {
        ...
        foreach ([self::LEGACY_POST_TYPE, self::FONT_FACE_POST_TYPE] as $postType) {
            ...
            foreach ($fontIds as $fontId) {
                ...
                $variants = $postType === self::LEGACY_POST_TYPE
                    ? get_post_meta($fontId, self::LEGACY_META_KEY, true)
                    : get_post_meta($fontId, 'font_face', true);

                if (! is_array($variants) || $variants === []) {
                    $variant = $this->font_face_variant($fontId);
                    $variants = $variant === [] ? [] : [$variant];
                } elseif (isset($variants['font_weight'])
                    || isset($variants['weight'])
                    || array_intersect(['woff2', 'woff', 'ttf', 'otf', 'eot'], array_keys($variants)) !== []) {
                    $variants = [$variants];
                }
    ...
    {
        $variant = [
            'font_weight' => $this->first_meta($fontId, ['font_face_weight', 'font_weight', 'weight'], '400'),
            'font_style' => $this->first_meta($fontId, ['font_face_style', 'font_style', 'style'], 'normal'),
            'font_stretch' => $this->first_meta($fontId, ['font_face_stretch', 'font_stretch', 'stretch'], 'normal'),
        ];
        $hasFile = false;

        foreach (['woff2', 'woff', 'ttf', 'otf', 'eot'] as $extension) {
            $source = $this->first_meta($fontId, [
                'font_face_file_' . $extension,
                'font_file_' . $extension,
                $extension,
            ], null);
    ...
    {
        foreach ($keys as $key) {
            $value = get_post_meta($fontId, $key, true);
