...
final class CustomAdobeFonts extends AbstractApi implements ApiInterface
{
    ...
    public function import_fonts(WP_REST_Request $request): WP_REST_Response
    {
        if (! defined('CUSTOM_TYPEKIT_FONTS_VER')) {
            return new WP_REST_Response([
                'message' => __('Custom Adobe Fonts is not activated.', 'jooosi-fon'),
            ], 404);
        }
        ...
        if (! is_array($source)) {
            return new WP_REST_Response([
                'message' => __('Custom Adobe Fonts records could not be read.', 'jooosi-fon'),
            ], 500);
        }
        ...
        if (! RequestValidator::projectId($kit['id']) || ! RequestValidator::adobeKit($kit) || $kit['families'] === []) {
            return new WP_REST_Response([
                'message' => __('The Custom Adobe Fonts project has no usable published families.', 'jooosi-fon'),
            ], 422);
        }
        ...
        if ($options === null) {
            return new WP_REST_Response([
                'message' => __('Jooosi Fon settings are invalid, so the Adobe project was not changed.', 'jooosi-fon'),
            ], 500);
        }

        $adobeOptions = isset($options['adobe_fonts']) && is_array($options['adobe_fonts'])
            ? $options['adobe_fonts']
            : [];
        ...
        if ($currentProjectId !== '' && $currentProjectId !== $kit['id']) {
            return new WP_REST_Response([
                'message' => __('Jooosi Fon is already connected to a different Adobe Fonts project. Disconnect it before migrating this source.', 'jooosi-fon'),
            ], 409);
        }
        ...
        try {
            $insertedIds = $repository->transaction(function (FontRepository $repository) use ($kit, $options): array {
                ...
                foreach ($kit['families'] as $family) {
                    $ids[] = $repository->insert([
                        'status' => 1,
                        'type' => 'adobe-fonts',
                        'title' => sanitize_text_field($family['name']),
                        'slug' => sanitize_key($family['id']),
                        'family' => sanitize_text_field($family['slug']),
                        'metadata' => FontCodec::encode($family),
                        'font_faces' => FontCodec::encode([]),
                    ], ['%d', '%s', '%s', '%s', '%s', '%s', '%s']);
                }
            ...
            return new WP_REST_Response([
                'message' => __('The Custom Adobe Fonts migration failed; the previous Adobe project and families were preserved.', 'jooosi-fon'),
            ], 500);
        }

        FontEvents::dispatch('migration', [
            'source' => 'custom-adobe-fonts',
            'ids' => $insertedIds,
        ], true);

        return new WP_REST_Response([
            'message' => __('Custom Adobe Fonts project and families imported. Source settings were preserved.', 'jooosi-fon'),
    ...
    public function clean_up(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if ($source !== null && ! delete_option(self::SOURCE_OPTION)) {
            return new WP_REST_Response([
                'message' => __('Custom Adobe Fonts cleanup failed and its source settings were preserved.', 'jooosi-fon'),
            ], 500);
        }
        ...
        return new WP_REST_Response([
            'message' => __('Custom Adobe Fonts source settings were removed. The migrated Jooosi Fon Adobe project remains connected.', 'jooosi-fon'),
            'cleaned' => is_array($source) ? 1 : 0,
        ]);
    }
    ...
    private function jooosi_options(): ?array
    {
        ...
        try {
            $options = json_decode($stored, true, 512, JSON_THROW_ON_ERROR);
