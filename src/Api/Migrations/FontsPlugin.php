...
final class FontsPlugin extends AbstractApi implements ApiInterface
{
    ...
    public function import_fonts(WP_REST_Request $request): WP_REST_Response
    {
        if (! $this->is_active()) {
            return new WP_REST_Response([
                'message' => __('Fonts Plugin is not activated.', 'jooosi-fon'),
            ], 404);
        }
        ...
        if (is_wp_error($terms)) {
            return new WP_REST_Response([
                'message' => __('Fonts Plugin records could not be read.', 'jooosi-fon'),
            ], 500);
        }
        ...
        $families = SourceFontNormalizer::fontsPlugin(
            $rows,
            $this->selector_assignments(),
            (string) get_theme_mod('ogf_font_display', 'swap')
        );
        $normalizedFaceCount = array_sum(array_map(
            static fn (array $family): int => count($family['faces']),
            $families
        ));
        ...
        try {
            foreach ($families as $sourceFamily) {
                ...
                $records[] = [
                    ...
                    'metadata' => FontCodec::encode([
                        'preload' => (bool) $sourceFamily['preload'],
                        'selector' => $sourceFamily['selector'],
                        'display' => $sourceFamily['display'],
                    ]),
            ...
            if ($skipped > 0) {
                ...
                return new WP_REST_Response([
                    'message' => __('Some Fonts Plugin variants had no supported font files. Nothing was imported so the source records remain safe.', 'jooosi-fon'),
                    'skipped' => $skipped,
                ], 422);
            }
            ...
            return new WP_REST_Response([
                'message' => __('The Fonts Plugin migration failed; newly created Jooosi Fon records were rolled back.', 'jooosi-fon'),
            ], 500);
        }
        ...
        return new WP_REST_Response([
            'message' => __('Fonts Plugin uploaded families imported. Source records were preserved.', 'jooosi-fon'),
    ...
    public function clean_up(WP_REST_Request $request): WP_REST_Response
    {
        ...
        if ($wpdb->query('START TRANSACTION') === false) {
            return new WP_REST_Response([
                'message' => __('Fonts Plugin cleanup could not start a database transaction.', 'jooosi-fon'),
            ], 500);
        }
        ...
        try {
            ...
            return new WP_REST_Response([
                'message' => __('Fonts Plugin cleanup failed and the source transaction was rolled back.', 'jooosi-fon'),
            ], 500);
        }
        ...
        return new WP_REST_Response([
            'message' => __('Fonts Plugin uploaded-font records and their selector assignments were removed. Original media files were left in place.', 'jooosi-fon'),
            'cleaned' => count($termIds),
        ]);
    }

    private function is_active(): bool
    {
        return defined('OGF_VERSION')
            && taxonomy_exists(self::TAXONOMY)
            && function_exists('ogf_get_elements')
            && function_exists('ogf_get_custom_elements');
    }
