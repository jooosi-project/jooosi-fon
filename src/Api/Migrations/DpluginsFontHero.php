...
class DpluginsFontHero extends AbstractApi implements ApiInterface
{
    ...
    public function import_fonts(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        ...
        if ($wpdb->last_error) {
            return new WP_REST_Response([
                'message' => __('Font Hero records could not be read.', 'jooosi-fon'),
            ], 500);
        }
        ...
        try {
            ...
            return new WP_REST_Response([
                'message' => __('The Font Hero migration failed; newly created Jooosi Fon records were rolled back.', 'jooosi-fon'),
            ], 500);
        }

        $media->commit();
        FontEvents::dispatch('migration', [
            'source' => 'font-hero-dplugins',
            'ids' => $insertedIds,
        ], true);

        return new WP_REST_Response([
            'message' => __('Font Hero fonts imported. Source records were preserved.', 'jooosi-fon'),
    ...
    public function clean_up(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        ...
        if ($wpdb->query('START TRANSACTION') === false) {
            return new WP_REST_Response([
                'message' => __('Font Hero cleanup could not start a database transaction.', 'jooosi-fon'),
            ], 500);
        }

        try {
            if ($wpdb->query(sprintf('DELETE FROM %sdp_fh_font_faces', $wpdb->prefix)) === false
                || $wpdb->query(sprintf('DELETE FROM %sdp_fh_fonts', $wpdb->prefix)) === false
            ...
            return new WP_REST_Response([
                'message' => __('Font Hero cleanup failed; the source transaction was rolled back.', 'jooosi-fon'),
            ], 500);
        }
        ...
        return new WP_REST_Response([
            'message' => __('Font Hero source records deleted.', 'jooosi-fon'),
        ]);
    }
}
