    ...
    public function import_fonts(WP_REST_Request $request): WP_REST_Response
    {
        if (! defined('BSF_CUSTOM_FONTS_POST_TYPE')) {
            return new WP_REST_Response([
                'message' => __('Custom Fonts by Brainstorm Force is not activated.', 'jooosi-fon'),
            ], 404);
        }
        ...
        try {
            foreach ($fontIds as $fontId) {
                ...
                $source = get_post_meta($fontId, 'fonts-data', true);
            ...
            if ($skipped > 0) {
                ...
                return new WP_REST_Response([
                    'message' => __('Some Brainstorm Force font records had no supported, readable files. Nothing was imported so the source records remain safe.', 'jooosi-fon'),
                    'skipped' => $skipped,
                ], 422);
            }
            ...
            return new WP_REST_Response([
                'message' => __('The Brainstorm Force Custom Fonts migration failed; newly created Jooosi Fon records were rolled back.', 'jooosi-fon'),
            ], 500);
        }

        $media->commit();
        FontEvents::dispatch('migration', [
            'source' => 'brainstorm-force-custom-fonts',
            'ids' => $insertedIds,
        ], true);

        return new WP_REST_Response([
            'message' => __('Brainstorm Force Custom Fonts imported. Source records were preserved.', 'jooosi-fon'),
    ...
    public function clean_up(WP_REST_Request $request): WP_REST_Response
    {
        ...
        foreach ($fontIds as $fontId) {
            ...
            if ($fontId < 1 || ! wp_trash_post($fontId)) {
                ...
                return new WP_REST_Response([
                    'message' => __('Brainstorm Force source cleanup failed and previously trashed records were restored.', 'jooosi-fon'),
                ], 500);
            }
        ...
        return new WP_REST_Response([
            'message' => __('Brainstorm Force Custom Fonts source records moved to the trash.', 'jooosi-fon'),
