...
final class UseAnyFont extends AbstractApi implements ApiInterface
{
    ...
    public function import_fonts(WP_REST_Request $request): WP_REST_Response
    {
        if (! $this->is_active()) {
            return new WP_REST_Response([
                'message' => __('Use Any Font is not activated.', 'jooosi-fon'),
            ], 404);
        }
        ...
        if ($fontRows === null || $assignments === null) {
            return new WP_REST_Response([
                'message' => __('Use Any Font records could not be read.', 'jooosi-fon'),
            ], 500);
        }
        ...
        if ($directory === '') {
            return new WP_REST_Response([
                'message' => __('Use Any Font storage could not be located.', 'jooosi-fon'),
            ], 500);
        }

        $families = SourceFontNormalizer::useAnyFont(
            $fontRows,
            $assignments,
            $directory,
            (string) get_option('uaf_font_display_property', 'auto')
        );
        ...
        $normalizedFaceCount = array_sum(array_map(
            static fn (array $sourceFamily): int => count($sourceFamily['faces']),
            $families
        ));
        ...
        try {
            ...
            if ($skipped > 0) {
                ...
                return new WP_REST_Response([
                    'message' => __('Some Use Any Font records had no supported, readable files. Nothing was imported so the source records remain safe.', 'jooosi-fon'),
                    'skipped' => $skipped,
                ], 422);
            }
            ...
            return new WP_REST_Response([
                'message' => __('The Use Any Font migration failed; newly created Jooosi Fon records were rolled back.', 'jooosi-fon'),
            ], 500);
        }
        ...
        return new WP_REST_Response([
            'message' => __('Use Any Font families imported. Source records were preserved.', 'jooosi-fon'),
    ...
    public function clean_up(WP_REST_Request $request): WP_REST_Response
    {
        ...
        try {
            ...
            return new WP_REST_Response([
                'message' => __('Use Any Font cleanup failed and the source options were restored.', 'jooosi-fon'),
            ], 500);
        }
        ...
        return new WP_REST_Response([
            'message' => __('Use Any Font source records and selector assignments were removed. Original font files were left in place.', 'jooosi-fon'),
            'cleaned' => $cleaned,
        ]);
    }
