...
class BricksCustomFonts extends AbstractApi implements ApiInterface
{
    ...
    public function import_fonts(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        ...
        try {
            ...
            return new WP_REST_Response([
                'message' => __('The Bricks migration failed; newly created Jooosi Fon records were rolled back.', 'jooosi-fon'),
            ], 500);
        }

        $media->commit();
        FontEvents::dispatch('migration', [
            'source' => 'bricks-custom-fonts',
            'ids' => $insertedIds,
        ], true);

        return new WP_REST_Response([
            'message' => __('Bricks fonts imported. Source records were preserved.', 'jooosi-fon'),
    ...
    public function clean_up(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        ...
        foreach ($font_ids as $font_id) {
            if (! wp_trash_post($font_id)) {
                ...
                return new WP_REST_Response([
                    'message' => __('Bricks source cleanup failed and previously trashed records were restored.', 'jooosi-fon'),
                ], 500);
            }
        ...
        return new WP_REST_Response([
            'message' => __('Bricks source records moved to the trash.', 'jooosi-fon'),
