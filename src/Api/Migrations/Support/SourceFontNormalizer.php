...
final class SourceFontNormalizer
{
    ...
    public static function brainstormForce(array $fontData): array
    {
        $faces = [];
        $skipped = 0;
        $variations = isset($fontData['variations']) && is_array($fontData['variations'])
            ? $fontData['variations']
            : [];
        ...
        foreach ($variations as $variation) {
            ...
            $faces[] = [
                'weight' => self::weight($weightSource),
                'width' => '100%',
                'style' => self::style((string) ($variation['font_style'] ?? ''), $weightSource),
                'files' => $files,
            ];
        }
        ...
        return [
            'display' => self::display(is_string($display) ? $display : 'auto'),
     ...
     * }>
     */
    public static function useAnyFont(
        array $fontRows,
        array $assignments,
        ...
        foreach ($fontRows as $sourceKey => $fontRow) {
            ...
            foreach (['woff2', 'woff', 'eot'] as $extension) {
                $files[] = [
                    'location' => $baseDirectory . $fontPath . '.' . $extension,
                    'extension' => $extension,
                ];
            }
            ...
            $families[$family]['faces'][] = [
                'source_key' => $key,
                'weight' => self::weight((string) ($fontRow['font_weight'] ?? '400')),
                'width' => self::width((string) ($fontRow['font_stretch'] ?? 'normal')),
                'style' => self::style((string) ($fontRow['font_style'] ?? '')),
                'files' => $files,
            ];
        }
    ...
    public static function fontsPlugin(array $fontRows, array $assignments, string $display): array
    {
        ...
        foreach ($fontRows as $fontRow) {
            ...
            $families[$family]['faces'][] = [
                ...
                'weight' => self::weight(self::stringValue($fontRow['weight'] ?? '400', '400')),
                'width' => self::width(self::stringValue($fontRow['stretch'] ?? 'normal', 'normal')),
                'style' => self::style(self::stringValue($fontRow['style'] ?? 'normal', 'normal')),
                'preload' => $preload,
                'files' => $files,
            ];
        }
    ...
    public static function elementor(array $variants, string $display = 'auto'): array
    {
        ...
        foreach ($variants as $variant) {
            ...
            foreach (self::SUPPORTED_EXTENSIONS as $extension) {
                ...
                if (is_array($source)) {
                    $source = [
                        ...
                        'id' => $source['id'] ?? $source['attachment_id'] ?? 0,
                    ];
                }
            ...
            $faces[] = [
                ...
                'width' => self::width((string) ($variant['font_stretch'] ?? $variant['stretch'] ?? 'normal')),
                'style' => self::style((string) ($variant['font_style'] ?? $variant['style'] ?? 'normal'), $fontWeight),
                'preload' => filter_var($variant['preload'] ?? false, FILTER_VALIDATE_BOOLEAN),
                'files' => $files,
            ];
        }
    ...
    public static function customAdobeKit(array $source): array
    {
        ...
        $details = isset($source['custom-typekit-font-details']) && is_array($source['custom-typekit-font-details'])
            ? $source['custom-typekit-font-details']
            : [];
        $families = [];
        ...
        foreach ($details as $sourceName => $detail) {
            ...
            $cssNames = array_values(array_filter(
                array_map('strval', (array) ($detail['css_names'] ?? [])),
            ...
            $weights = array_values(array_unique(array_map(
                static fn ($weight): string => self::weight((string) $weight),
                (array) ($detail['weights'] ?? [])
            )));
            $families[] = [
                'id' => $slug,
                'name' => $name,
                'slug' => $cssNames[0] ?? $slug,
                'css_names' => $cssNames,
                'css_stack' => trim((string) ($detail['fallback'] ?? '')),
                'weights' => $weights,
            ];
        }
    ...
    private static function remoteFiles($urls): array
    {
        ...
        foreach ($urls as $url) {
            ...
            if (! in_array($extension, self::SUPPORTED_EXTENSIONS, true) || isset($seenExtensions[$extension])) {
                continue;
            }
    ...
    private static function display(string $display): string
    {
        ...
        return in_array($display, ['auto', 'block', 'swap', 'fallback', 'optional'], true) ? $display : 'auto';
    }
    ...
    private static function weight(string $weight): string
    {
        ...
        if ($minimum > 1_000 || ($maximum !== null && ($maximum > 1_000 || $minimum > $maximum))) {
            return '400';
        }
    ...
    private static function selector(string $selector): string
    {
        ...
        if ($selector === ''
            || strlen($selector) > 1_000
            || preg_match('/[<{};@]|\/\*|\*\/|[\x00-\x08\x0B\x0C\x0E-\x1F]/', $selector) !== 0) {
            return '';
        }
