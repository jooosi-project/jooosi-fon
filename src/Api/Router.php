...
class Router
{
    ...
    public function scan_apis(): void
    {
        ...
        $classes = apply_filters('f!jooosi/fon/api/router:classes', [
            'fonts' => Font::class,
            'setting/adobe-fonts' => AdobeFonts::class,
            'setting/cache' => Cache::class,
            'setting/license' => License::class,
            'setting/option' => Option::class,
            'migrations/bricks-custom-fonts' => BricksCustomFonts::class,
            'migrations/brainstorm-force-custom-fonts' => BrainstormForceCustomFonts::class,
            'migrations/custom-adobe-fonts' => CustomAdobeFonts::class,
            'migrations/elementor-pro-custom-fonts' => ElementorProCustomFonts::class,
            'migrations/font-hero-dplugins' => DpluginsFontHero::class,
            'migrations/fonts-plugin' => FontsPlugin::class,
            'migrations/use-any-font' => UseAnyFont::class,
        ]);
        $classes = apply_filters_deprecated(
            'f!yabe/webfont/api/router:classes',
            [$classes],
            '2.1.0',
            'f!jooosi/fon/api/router:classes'
        );
    ...
    public function register_apis(): void
    {
        ...
        $apis = apply_filters_deprecated(
            'f!yabe/webfont/api/router:register_apis',
            [$apis],
            '2.1.0',
            'f!jooosi/fon/api/router:register_apis'
        );

        foreach ((array) $apis as $api) {
            $className = $api['class_name'] ?? null;

            if (! is_string($className) || ! is_a($className, ApiInterface::class, true)) {
    ...
    {
        if ($prefix === '' || ! is_string($className) || ! is_a($className, ApiInterface::class, true)) {
    ...
    private function scanLegacyExtensionDirectories(): void
    {
        ...
        foreach (get_declared_classes() as $className) {
            if (! is_a($className, ApiInterface::class, true)) {
                continue;
            }
