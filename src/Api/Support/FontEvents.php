...
final class FontEvents
{
     ...
     * @param mixed $payload
     */
    public static function dispatch(string $event, $payload = null, bool $async = true): void
    {
     ...
     * @param mixed $payload
     */
    public static function invalidate(string $source, $payload = null, bool $async = true): void
    {
        Font::clear_cache();
    ...
    {
        $hook = $async
            ? 'a!jooosi/fon/api/font:fonts_event_async'
            : 'a!jooosi/fon/api/font:fonts_event';
        $legacyHook = $async
            ? 'a!yabe/webfont/api/font:fonts_event_async'
            : 'a!yabe/webfont/api/font:fonts_event';

        do_action($hook, $source, $payload);
        do_action_deprecated(
            $legacyHook,
            [str_replace('!jooosi/fon/', '!yabe/webfont/', $source), $payload],
            '2.1.0',
            $hook
        );
    }
