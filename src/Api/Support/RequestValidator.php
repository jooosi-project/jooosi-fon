...
final class RequestValidator
{
    ...
    {
        return is_string($value) && in_array($value, ['custom', 'google-fonts', 'adobe-fonts'], true);
    }
    ...
    public static function subsets($value): bool
    {
        return is_string($value)
            && ($value === '' || preg_match('/^[a-zA-Z0-9_-]+(?:,[a-zA-Z0-9_-]+)*$/', $value) === 1);
    }

    public static function importItem($value): bool
    {
        return is_array($value)
            && isset($value['type'], $value['title'], $value['family'], $value['font_faces'], $value['metadata'])
            && self::fontType($value['type'])
            && self::nonEmptyString($value['title'])
            && self::nonEmptyString($value['family'])
            && is_string($value['font_faces'])
            && is_string($value['metadata'])
            && strlen($value['font_faces']) <= 30 * 1024 * 1024
            && strlen($value['metadata']) <= 5 * 1024 * 1024;
    }
    ...
    public static function googleMetadata($value): bool
    {
        ...
        if (! isset($google['font_data'], $google['font_faces'], $google['font_files'], $google['formats'], $google['subsets'])
            || ! is_array($google['font_data'])
            || ! is_array($google['font_faces'])
            || ! is_array($google['font_files'])
            || ! is_array($google['formats'])
            || ! is_array($google['subsets'])
            || ! isset($google['font_data']['family'], $google['font_data']['slug'])
            || ! self::nonEmptyString($google['font_data']['family'])
            || ! self::nonEmptyString($google['font_data']['slug'])
            || strlen($google['font_data']['family']) > 255
            || strlen($google['font_data']['slug']) > 255
            || count($google['font_faces']) > 100
            || count($google['font_files']) > 500
            || count($google['formats']) > 3
            || count($google['subsets']) > 50
            || $google['formats'] === []
            || $google['subsets'] === []
            || $google['font_files'] === []
            || $google['font_faces'] === []) {
        ...
        foreach ($google['formats'] as $format) {
            if (! in_array($format, ['woff2', 'woff', 'ttf'], true)) {
                return false;
            }
        }
        ...
        foreach ($google['font_files'] as $file) {
            if (! is_array($file)
                || ! isset($file['uid'], $file['format'], $file['weight'], $file['style'], $file['subsets'], $file['url'])
                || ! self::nonEmptyString($file['uid'])
                || strlen($file['uid']) > 255
                || ! in_array($file['format'], ['woff2', 'woff', 'ttf'], true)
                || ! is_numeric($file['weight'])
                || ! is_string($file['style'])
                || ! is_array($file['subsets'])
                || count($file['subsets']) > 50
                || ! filter_var($file['url'], FILTER_VALIDATE_URL)
                || strtolower((string) parse_url($file['url'], PHP_URL_SCHEME)) !== 'https'
                || strtolower((string) parse_url($file['url'], PHP_URL_HOST)) !== 'fonts.gstatic.com') {
                return false;
            }
        ...
        foreach ($google['font_faces'] as $face) {
            if (! is_array($face)
                || ! array_key_exists('weight', $face)
                || ! array_key_exists('width', $face)
                || ! array_key_exists('style', $face)
                || ! array_key_exists('isEnabled', $face)
                || ! array_key_exists('display', $face)
                || ! array_key_exists('selector', $face)
                || ! array_key_exists('comment', $face)
                || ! array_key_exists('preload', $face)
                || ! is_numeric($face['weight'])
                || ! is_string($face['style'])
                || ! self::boolean($face['isEnabled'])) {
                return false;
            }

            $hasEnabledFace = $hasEnabledFace || filter_var($face['isEnabled'], FILTER_VALIDATE_BOOLEAN) === true;
        }

        if (! empty($google['variable'])
            && (! isset($google['font_data']['axes']) || ! is_array($google['font_data']['axes']))) {
            return false;
        }
        ...
        if (! empty($google['variable'])) {
            ...
            foreach ($google['font_data']['axes'] as $axis) {
                if (! is_array($axis)
                    || ! isset($axis['tag'], $axis['min'], $axis['max'])
                    || ! is_string($axis['tag'])
                    || preg_match('/^[a-zA-Z0-9]{1,10}$/', $axis['tag']) !== 1
                    || ! is_numeric($axis['min'])
                    || ! is_numeric($axis['max'])
    ...
    public static function fontFaces($value): bool
    {
        ...
        foreach ($value as $face) {
            if (! is_array($face)
                || ! isset($face['id'], $face['weight'], $face['style'], $face['files'])
                || ! self::nonEmptyString($face['id'])
                || ! self::fontWeight($face['weight'])
                || ! is_string($face['style'])
                || (array_key_exists('isEnabled', $face) && ! self::boolean($face['isEnabled']))
                || ! is_array($face['files'])
                || count($face['files']) > 10) {
                return false;
            }
    ...
    {
        return is_array($value)
            && isset($value['attachment_id'], $value['extension'])
            && self::positiveInteger($value['attachment_id'])
            && self::fontExtension($value['extension']) !== null;
    }
    ...
    public static function fontWeight($value): bool
    {
        ...
        foreach ($weights as $weight) {
            if (is_numeric($weight) && ((int) $weight < 1 || (int) $weight > 1_000)) {
                return false;
            }
        }
    ...
    public static function adobeKit($value): bool
    {
        ...
        foreach ($value['families'] as $family) {
            if (! is_array($family)
                || ! isset($family['id'], $family['name'], $family['slug'])
                || ! self::nonEmptyString($family['id'])
                || ! self::nonEmptyString($family['name'])
                || ! self::nonEmptyString($family['slug'])) {
                return false;
            }
        }
