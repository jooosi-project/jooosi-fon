...
final class GoogleFontFileSelector
{
        ...
        foreach ($subsets as $subset) {
            foreach ($files as $file) {
                if (self::matchesFace($file, $face, $formats)
                    && in_array($subset, (array) ($file['subsets'] ?? []), true)) {
                    $selected[] = $file;
                }
            }
        }

        $usesNamedSubsets = array_reduce(
            $subsets,
            static fn (bool $carry, string $subset): bool => $carry && preg_match('/\d/', $subset) !== 1,
            true
        );
    ...
    {
        return ($file['weight'] ?? null) === ($face['weight'] ?? null)
            && ($file['style'] ?? null) === ($face['style'] ?? null)
            && in_array($file['format'] ?? null, $formats, true);
    }
}
