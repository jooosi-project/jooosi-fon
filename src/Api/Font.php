...
class Font extends AbstractApi implements ApiInterface
{
    ...
    public function register_custom_endpoints(): void
    {
        $id_args = [
            'id' => [
                ...
                'validate_callback' => [RequestValidator::class, 'positiveInteger'],
            ],
        ];
        $custom_font_args = [
            'title' => [
                ...
                'validate_callback' => [RequestValidator::class, 'nonEmptyString'],
            ],
            'family' => [
                'required' => true,
                'sanitize_callback' => 'sanitize_text_field',
            ...
            'status' => [
                ...
                'validate_callback' => [RequestValidator::class, 'boolean'],
            ],
            'metadata' => [
                'required' => true,
                'validate_callback' => [RequestValidator::class, 'arrayValue'],
            ],
            'font_faces' => [
                'required' => true,
                'sanitize_callback' => [RequestValidator::class, 'sanitizeFontFaces'],
                'validate_callback' => [RequestValidator::class, 'fontFaces'],
            ],
        ];
        $google_font_args = [
            'title' => $custom_font_args['title'],
            'status' => $custom_font_args['status'],
            'metadata' => [
                'required' => true,
                'validate_callback' => [RequestValidator::class, 'googleMetadata'],
            ],
        ];
            ...
            [
                ...
                'args' => [
                    ...
                    'per_page' => [
                        'default' => 20,
                        'sanitize_callback' => 'absint',
                        'validate_callback' => [RequestValidator::class, 'pageSize'],
                    ],
                    'search' => [
                        'default' => '',
                        'sanitize_callback' => 'sanitize_text_field',
        ...
        register_rest_route(
            ...
            [
                ...
                'args' => [
                    'items' => [
                        'required' => true,
                        'validate_callback' => [RequestValidator::class, 'positiveIntegerList'],
                        'sanitize_callback' => static fn ($items): array => array_values(array_unique(array_map('absint', $items))),
                    ],
                ],
            ]
        );
        ...
        register_rest_route(
            ...
            [
                ...
                'args' => [
                    ...
                    'item' => [
                        'required' => true,
                        'validate_callback' => [RequestValidator::class, 'importItem'],
                    ],
                ],
            ]
        );
        ...
        register_rest_route(
            ...
            [
                ...
                'permission_callback' => fn (\WP_REST_Request $wprestRequest): bool => $this->permission_callback($wprestRequest),
                'args' => array_merge($id_args, $custom_font_args),
            ]
        );
        ...
        register_rest_route(
            ...
            [
                ...
                'permission_callback' => fn (\WP_REST_Request $wprestRequest): bool => $this->permission_callback($wprestRequest),
                'args' => array_merge($id_args, $google_font_args),
            ]
        );
            ...
            [
                ...
                'args' => [
                    ...
                    'subsets' => [
                        'default' => '',
                        'sanitize_callback' => 'sanitize_text_field',
                        'validate_callback' => [RequestValidator::class, 'subsets'],
                    ],
                ],
            ]
        );
    ...
    private function index(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        ...
        return new WP_REST_Response([
            ...
            'meta' => [
                ...
                'infrastructure' => [
                    'google_fonts' => $this->get_google_fonts_catalog_stats(),
                    'active_fonts' => [
                        'total' => $total_active,
                    ],
                    'adobe_fonts' => [
                        'active' => trim((string) Config::get('adobe_fonts.project_id', '')) !== '',
                        'total' => $total_adobe_fonts,
                    ],
                    'integrations' => [
                        'total' => Integration::get_registered_count(),
                    ],
                ],
            ],
        ], 200, [
    ...
    private function get_google_fonts_catalog_stats(): array
    {
        ...
        $source_path = $has_cache
            ? $cache_path
            : dirname(JOOOSI_FON::FILE) . '/google-fonts.json';
        ...
        if (
            is_array($cached)
            && ($cached['source_path'] ?? null) === $source_path
            && ($cached['source_updated_at'] ?? null) === $source_updated_at
        ) {
            return [
        ...
        if (is_readable($source_path)) {
            try {
                $catalog = json_decode(file_get_contents($source_path), true, 512, JSON_THROW_ON_ERROR);
    ...
    private function custom_store(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        try {
            ...
            $id = $repository->insert([
                'type' => 'custom',
                'title' => (string) $wprestRequest->get_param('title'),
                'slug' => Common::random_slug(10),
                'family' => (string) $wprestRequest->get_param('family'),
                'status' => rest_sanitize_boolean($wprestRequest->get_param('status')),
                'metadata' => FontCodec::encode((array) $wprestRequest->get_param('metadata')),
                'font_faces' => FontCodec::encode((array) $wprestRequest->get_param('font_faces')),
            ], ['%s', '%s', '%s', '%s', '%d', '%s', '%s']);
    ...
    private function update_status(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        ...
        if (! $repository->find($id)) {
            return new WP_REST_Response([
                'message' => __('Font not found', 'jooosi-fon'),
            ], 404, []);
        }
    ...
    private function custom_update(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        ...
        try {
            ...
            $repository->update($id, [
                ...
                'status' => rest_sanitize_boolean($wprestRequest->get_param('status')),
                'metadata' => FontCodec::encode((array) $wprestRequest->get_param('metadata')),
                'font_faces' => FontCodec::encode($fontFaces),
            ], ['%s', '%s', '%d', '%s', '%s']);
    ...
    private function google_fonts_store(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        ...
        foreach ($m_font_faces as $k => $m_face) {
            ...
            if ($metadata['google_fonts']['variable']) {
                ...
                $all_filtered_m_font_files = GoogleFontFileSelector::variableFiles(
                    $m_font_files,
                    $m_face,
                    $metadata['google_fonts']['subsets'],
                    $metadata['google_fonts']['formats']
                );
        ...
        try {
            ...
            $id = $repository->insert([
                ...
                'family' => $family,
                'status' => $status,
                'metadata' => FontCodec::encode($metadata),
                'font_faces' => FontCodec::encode($font_faces),
            ], ['%s', '%s', '%s', '%s', '%d', '%s', '%s']);
            $media->commit();
        } catch (\Throwable $throwable) {
            $media->rollback();
    ...
    private function export(WP_REST_Request $wprestRequest): WP_REST_Response
    {
                        ...
                        if ($is_bundled) {
                            ...
                            if ($file_path) {
                                ...
                                if (! is_int($fileSize) || $fileSize < 1) {
                                    return new WP_REST_Response([
                                        'message' => __('A font attachment is missing or unreadable.', 'jooosi-fon'),
                                    ], 409);
                                }
                                ...
                                if ($bundledBytes > 50 * MB_IN_BYTES) {
                                    return new WP_REST_Response([
                                        'message' => __('The bundled export exceeds the 50 MB limit. Export fewer fonts or disable bundled binaries.', 'jooosi-fon'),
                                    ], 413);
                                }
                                ...
                                if (! is_string($binary)) {
                                    return new WP_REST_Response([
                                        'message' => __('A font attachment could not be read.', 'jooosi-fon'),
                                    ], 409);
                                }
        ...
        $data = [
            'module_id' => JOOOSI_FON::WP_OPTION,
            'version' => JOOOSI_FON::VERSION,
            'export_time' => time(),
            'site_url' => site_url(),
    ...
    private function import(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        ...
        try {
            $encodedFontFaces = base64_decode($item['font_faces'], true);
            $encodedMetadata = base64_decode($item['metadata'], true);
        ...
        if ($item['type'] === 'adobe-fonts') {
            ...
            $font_faces = json_decode($encodedFontFaces, true, 512, JSON_THROW_ON_ERROR);
            $metadata = json_decode($encodedMetadata, true, 512, JSON_THROW_ON_ERROR);
            ...
            try {
                ...
                foreach ($font_faces as $i => $font_face) {
                    foreach ($font_face->files as $j => $file) {
                        ...
                        if ($is_bundled) {
                            $binary = base64_decode((string) ($file->binary ?? ''), true);
                            ...
                            if ($attachmentPath === ''
                                || $attachmentPath[0] !== '/'
                                || wp_parse_url($attachmentPath, PHP_URL_HOST) !== null) {
                                throw new \UnexpectedValueException('The imported attachment path is invalid.');
                            }

                            $attachment = $media->remote(
                                untrailingslashit($site_url) . $attachmentPath,
                                $file_name,
    ...
    private function google_fonts_metadata(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        ...
        $enable_update = apply_filters('f!jooosi/fon/font:google_fonts.metadata.enable_update', true);
    ...
    private function google_fonts_webfonts(WP_REST_Request $wprestRequest): WP_REST_Response
    {
        ...
        if (trim($slug) === '') {
            return new WP_REST_Response([
                'message' => __('Slug is required', 'jooosi-fon'),
            ], 400, []);
        }
        ...
        if ($subsets === []) {
            return new WP_REST_Response([
                'message' => __('None of the requested subsets are available for this font.', 'jooosi-fon'),
            ], 400, []);
        }
