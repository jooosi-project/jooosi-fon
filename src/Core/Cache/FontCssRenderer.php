...
final class FontCssRenderer
{
    ...
    public function render(array $fonts, string $adobeCss = ''): string
    {
        ...
        foreach ($fonts as $font) {
            foreach ($font['faces'] as $face) {
                ...
                $sources = array_map(function (array $file): string {
                    return sprintf(
                        "url('%s') format('%s')",
                        $this->escapeString($file['url']),
                        $this->escapeString($file['format'])
                    );
                }, $face['files']);
        ...
        foreach ($fonts as $font) {
            ...
            $variables[$font['custom_property']] = [
                'legacy_property' => $font['legacy_custom_property'],
                'value' => sprintf("'%s'%s", $this->escapeString($font['family']), $fallback),
            ];
        }
