...
final class FontCacheSnapshotBuilder
{
    ...
    public function build(): array
    {
        ...
        foreach ($rows as $row) {
            try {
                $metadata = FontCodec::decode((string) $row->metadata, true);
                $fontFaces = FontCodec::decode((string) $row->font_faces, true);
            ...
            foreach ($fontFaces as $faceIndex => $fontFace) {
                ...
                if (array_key_exists('isEnabled', $fontFace)
                    && filter_var($fontFace['isEnabled'], FILTER_VALIDATE_BOOLEAN) !== true) {
                    continue;
                }
            ...
            $fonts[] = [
                ...
                'family' => $family,
                'custom_property' => Font::css_custom_property($family),
                'legacy_custom_property' => Font::legacy_css_custom_property($family),
                'display' => $this->display((string) ($metadata['display'] ?? 'auto'), 'auto'),
                'preload' => filter_var($metadata['preload'] ?? false, FILTER_VALIDATE_BOOLEAN),
                'selector' => $selector,
                'fallback' => $fallback,
                'faces' => $faces,
            ];
    ...
    private function normalizeFace(array $face, int $fontId, int $faceIndex, array &$warnings): ?array
    {
        ...
        return [
            'weight' => $this->weight($face['weight'] ?? 400),
            'width' => $this->width($face['width'] ?? '100%'),
            'style' => $this->style((string) ($face['style'] ?? 'normal')),
            'display' => $this->display((string) ($face['display'] ?? ''), ''),
            'selector' => $this->selector((string) ($face['selector'] ?? '')),
            'comment' => $this->comment((string) ($face['comment'] ?? '')),
            'unicode_range' => $this->unicodeRange((string) ($face['unicodeRange'] ?? '')),
            'preload' => filter_var($face['preload'] ?? false, FILTER_VALIDATE_BOOLEAN),
            'files' => $files,
        ];
    }
    ...
    private function fontSelector(string $value): array
    {
        ...
        if ($fallback !== ''
            && (strlen($fallback) > 500
                || preg_match('/[^a-zA-Z0-9\s,_\'"-]/', $fallback) !== 0
                || substr_count($fallback, "'") % 2 !== 0
                || substr_count($fallback, '"') % 2 !== 0)) {
            $fallback = '';
        }
    ...
    private function selector(string $value): string
    {
        ...
        if ($value === ''
            || strlen($value) > 1_000
            || preg_match('/[<{};@]|\/\*|\*\/|[\x00-\x08\x0B\x0C\x0E-\x1F]/', $value) !== 0) {
            return '';
        }
    ...
    private function style(string $value): string
    {
        ...
        return preg_match('/^(?:normal|italic|oblique(?:\s+-?\d+(?:\.\d+)?deg(?:\s+-?\d+(?:\.\d+)?deg)?)?)$/', $value)
            ? $value
            : 'normal';
    }
    ...
    private function weight($value): string
    {
        ...
        if (in_array($value, ['normal', 'bold'], true)) {
            return $value;
        }
        ...
        foreach (preg_split('/\s+/', $value) ?: [] as $weight) {
            if ((int) $weight < 1 || (int) $weight > 1_000) {
                return '400';
            }
        }
    ...
    private function display(string $value, string $fallback): string
    {
        ...
        return in_array($value, ['auto', 'block', 'swap', 'fallback', 'optional'], true)
            ? $value
            : $fallback;
    }
    ...
    private function unicodeRange(string $value): string
    {
        ...
        return preg_match('/^U\+[0-9A-F?]{1,6}(?:-[0-9A-F]{1,6})?(?:\s*,\s*U\+[0-9A-F?]{1,6}(?:-[0-9A-F]{1,6})?)*$/i', $value)
            ? $value
            : '';
    }
