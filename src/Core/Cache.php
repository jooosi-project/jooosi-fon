...
class Cache
{
    ...
    public function schedule_cache(): void
    {
        ...
        if (! wp_next_scheduled('a!jooosi/fon/core/cache:build_cache')) {
            ...
            if ($scheduled === false) {
                self::updateBuildStatus([
                    'state' => 'failed',
                    'error' => 'The cache build could not be scheduled.',
                    'completed_at' => time(),
                ]);
            }
        }
    ...
    public function build_cache(): void
    {
        try {
            ...
            Notice::error($message, 'jooosi-fon-cache-build', true);
            return;
        }
        ...
        try {
            ...
            $changed = ! self::contentsMatch($cssPath, $payload)
                || ! self::contentsMatch($preloadPath, $artifacts['preload']);
            ...
            self::updateBuildStatus([
                'state' => 'succeeded',
                'completed_at' => time(),
                'hash' => hash('sha256', $payload . "\0" . $artifacts['preload']),
                'changed' => $changed,
    ...
    public static function get_kit_css($kit_id, ?array &$warnings = null): string
    {
        ...
        $response = wp_safe_remote_get(sprintf('https://use.typekit.net/%s.css', rawurlencode($kitId)), [
            'timeout' => 15,
            'redirection' => 2,
        ...
        if (is_wp_error($response)) {
            return self::adobeFallback(
                $cachedCss,
                $warnings,
                'Adobe Fonts could not be refreshed.'
            );
        }
        ...
        if ($status !== 200) {
            return self::adobeFallback(
                $cachedCss,
                $warnings,
                sprintf('Adobe Fonts returned HTTP %d.', $status)
            );
        }
        ...
        if (! is_string($body) || ! self::isSafeStylesheet($body)) {
            return self::adobeFallback(
                $cachedCss,
                $warnings,
                'Adobe Fonts returned an invalid stylesheet.'
            );
        }
    ...
    public static function get_kit_js($kit_id): string
    {
        ...
        $response = wp_safe_remote_get(sprintf('https://use.typekit.net/%s.js', rawurlencode($kitId)), [
            'timeout' => 15,
            'redirection' => 2,
            'limit_response_size' => 2 * MB_IN_BYTES,
        ]);
    ...
    private static function buildArtifacts(): array
    {
        ...
        $hasAdobeFonts = in_array('adobe-fonts', array_column($snapshot['fonts'], 'type'), true);
        ...
        $css = apply_filters_deprecated(
            'f!yabe/webfont/core/cache:build_css.append_content',
            [$css, $snapshot['rows']],
            '2.1.0',
            'f!jooosi/fon/core/cache:build_css.append_content'
        );
    ...
    {
        update_option(
            JOOOSI_FON::WP_OPTION . '_cache_build_status',
            array_merge(self::getBuildStatus(), $status),
            false
        );
    }
